 ...
 * @prop {CompiledSurfacePrompts} lastResponse The last compiled response given to the user
 * @prop {number} answer The actual number answer
 * @prop {number} guessCount Number of guesses made by the user so far
 * @prop {number} fallbackCount How many not understandable answers the user has given
 * @prop {number} steamSoundCount Times the steam sound was used before so it's not repeated
...
/**
 * Developer provided complex element that is like a client library SimpleResponse
 ...
 * @prop {(string | SimpleVariants)=} cardText The text that goes on the bottom of the image card
...
/**
 * Developer provided response that contains both visual and audio surface responses
 * @typedef {Object} SurfacePrompts
 * @prop {Prompt} visual Visual surface Prompt
 * @prop {Prompt=} audio Optional Audio surface Prompt: uses Visual prompts if not provided
 */

/**
 * Developer provided response data for a surface
 * @typedef {Object} Prompt
 * @prop {Elements} elements The array of developer provided elements for the compiler to process
 * @prop {(Array<string> | function(AppData): Array<string>)=} suggestions Array of strings
 *   or a function accepting app.data that returns an array of strings that are the suggestions
 *   for the response
 */

/**
 * The array of developer provided elements for the compiler to process
 * Each Element represents a RichResponse component like a SimpleResponse or a Card
 ...
 * PromptElement: A single extended data structure SimpleResponse or a string that is converted
 * ElementVariants: An array of PromptElements that has one variant randomly selected and saved
 *   by the compiler for future repeat usage
 * Array<ElementVariants>: An array of ElementVariants that is processed by the compiler to first
 *   randomly select a variant then concatenated together to form one single SimpleResponse
